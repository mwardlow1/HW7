...
public class ProblemSolutions {
    ...
    public static void selectionSort(int[] values, boolean ascending ) {
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            // Find the index of the min or max element in the unsorted portion
            for (int j = i + 1; j < n; j++) {
                if (ascending) {
                    // Ascending sort, look for the smallest value
                    if (values[j] < values[selectedIndex]) {
                        selectedIndex = j;      // Update index of smallest value
                    }
                } else {
                    // Descending sort, look for the largest value
                    if (values[j] > values[selectedIndex]) {
                        selectedIndex = j;      // Update index of the largest value
                    }
                }
            }

            // Swap the found min/max element with the element at index i
            int temp = values[i];
    ...
    public static boolean asteroidsDestroyed(int mass, int[] asteroids) {
        ...
        // Try to destroy each asteroid one at a time
        for (int asteroid : asteroids) {
            if (mass >= asteroid) {
                // Absord the asteroid and increase the planet's mass
                mass += asteroid;
            } else {
                return false;       // Asteroid is too big, the planet is destroyed
            }
        }
    ...
    public static int numRescueSleds(int[] people, int limit) {
        ...
        while (left <= right) {
            ...
            if (people[left] + people[right] <= limit) {
                left++;     // Send the lightest one
            }

            // Either way, send the heaviest one (alone or paired)
            right--;
            sleds++;
        }
